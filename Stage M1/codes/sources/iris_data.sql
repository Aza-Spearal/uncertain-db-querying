...
CREATE OR REPLACE FUNCTION fsepal_l(x float, y float) RETURNS bit(3) AS $$
    select case
        when x=y then b'11'
        when abs(x-y)<1 then b'10'
        else b'00'
    end
$$ LANGUAGE sql;

CREATE OR REPLACE FUNCTION fsepal_w(x float, y float) RETURNS bit(3) AS $$
    select case
        when x=y then b'11'
        when abs(x-y)<0.5 then b'10'
        else b'00'
    end
$$ LANGUAGE sql;

CREATE OR REPLACE FUNCTION fpetal_l(x float, y float) RETURNS bit(3) AS $$
    select case
        when x=y then b'11'
        when abs(x-y)<1 then b'10'
        else b'00'
...
CREATE OR REPLACE FUNCTION fpetal_w(x float, y float) RETURNS bit(3) AS $$
    select case
        when x=y then b'11'
        when abs(x-y)<0.5 then b'10'
        else b'00'
...
CREATE OR REPLACE FUNCTION fclass(x text, y text) RETURNS bit(1) AS $$
    select case
        when x=y then b'1'
        else b'0'
    END
...
CREATE OR REPLACE FUNCTION interprsepal_l(b bit(2)) RETURNS boolean AS $$
    select case
...
CREATE OR REPLACE FUNCTION interprsepal_w(b bit(2)) RETURNS boolean AS $$
    select case
        when b='11' or b='10' then true
        else false
    end
...
CREATE OR REPLACE FUNCTION interprpetal_l(b bit(2)) RETURNS boolean AS $$
    select case
        when b='11' or b='10' then true
        else false
    END
...
CREATE OR REPLACE FUNCTION interprpetal_w(b bit(2)) RETURNS boolean AS $$
    select case
        when b='11' or b='10' then true
        else false
    END
...
CREATE OR REPLACE FUNCTION interprclass(b bit(1)) RETURNS boolean AS $$
    select case
        when b='1' then true
        else false
    END
